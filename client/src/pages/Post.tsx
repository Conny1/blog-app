...
const Post = () => {
  ...
  const { data: similarPost, isLoading: ISsimilarPostLoading } =
    useGetBypostCategoryQuery(data ? data[0]?.postCategory : "random");

  if (isLoading || !data) return <>Loading...</>;

  const sanitizedHtml = sanitizeHtml(data[0]?.postDesc);
  return (
    <ParentContainer>
      <NavAndFooterBar>
        <Nav />
      ...
      <Container>
        ...
        <Recomentation>
          <Title style={{ fontSize: "25px", marginBottom: 0 }}>
            Similar Posts
          </Title>
          ...
          {ISsimilarPostLoading ? (
            ...
            <Title>No recomended Posts </Title>
          ) : (
